...
model_mask.load_state_dict(torch.load(os.path.join(HyperParameter.SAVE_DIR2,"best_model_mask.pth"),map_location=device))
model_mask.eval()

model_age.load_state_dict(torch.load(os.path.join(HyperParameter.SAVE_DIR2,"best_model_age.pth"),map_location=device))
model_age.eval()

model_gender.load_state_dict(torch.load(os.path.join(HyperParameter.SAVE_DIR2,"best_model_gender.pth"),map_location=device))
model_gender.eval()

transform= A.Compose([
    A.Resize(HyperParameter.RESIZE[0],HyperParameter.RESIZE[1]),
    A.HorizontalFlip(),
    A.CLAHE(),
    A.ColorJitter(),
    A.CoarseDropout(always_apply=False, p=0.5, max_holes=5, max_height=15, max_width=15, min_holes=1, min_height=8, min_width=8),
    A.Normalize(),
    ToTensorV2,
])
...
train_dataset=SplitByHumanDataset(train_idx, transform_train)
val_dataset=SplitByHumanDataset(val_idx, transform_val)
